    ...
    class UserRoutesSpec extends AnyWordSpec with Matchers with ScalaFutures with ScalatestRouteTest {
        ...
        "UserRoutes" should {
            "return no users if no present (GET /users)" in {
                val request = HttpRequest(uri = "http://localhost:1112/users")
            ...
            "be able to add users (POST /users)" in {
                ...
                val request = Post("http://localhost:1112/users").withEntity(userEntity)
                request ~> routes ~> check {
            ...
            "be able to remove users (DELETE /users)" in {
                val request = Delete(uri = "http://localhost:1112/users/Kapi")
                request ~> routes ~> check {
                    status should ===(StatusCodes.OK)
                    contentType should ===(ContentTypes.`application/json`)
